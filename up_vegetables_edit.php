...
<?php
    if(isset($_GET['id'])){
      ...
      $stmt = $conn->prepare("SELECT* FROM vegetable WHERE id=?");
...
<form action="up_vegetables_edit_db.php" method="POST" enctype="multipart/form-data" >
<div class="container">
<div class="row"><br><br>
...
<h4>กรอกข้อมูลการติดตามผัก</h4>
				...
				<input type="text" class="form-control" name ="name_vegetable" placeholder="ชื่อผัก" required /value="<?= $row['name_vegetable'];?>">
			</div>
			<div class="col-md-6" id="datetimepicker1">
				...
				<input type="date" class="form-control"  name="plan_date" placeholder="กรอกจำนวนวันที่ปลูก" required /value="<?= $row['plan_date'];?>">
			</div>
			<div class="col-md-6">
				<label for="form-label" class="form-label"><i class="bi bi-app-indicator"></i> อายุการปลูกผัก :<span style="color: red;">&nbsp;*</span></label>
				<input type="number" class="form-control"  name="harvest_age" placeholder="กรอกอายุการปลูก" required /value="<?= $row['harvest_age'];?>">
			</div>
            <div class="col-md-6">
				<label for="form-label" class="form-label"><i class="bi bi-alarm"></i> เหลือจำนวนวัน :<span style="color: red;">&nbsp;*</span></label>
				<input type="number" class="form-control"  name="how_day" placeholder="จำนวนวันที่ใกล้จะเก็บ" required /value="<?= $row['how_day'];?>">
			</div>
			<div class="col-md-6" id="datetimepicker1">
				<label for="exampleInputEmail1" class="form-label"><i class="bi bi-calendar"></i> วันที่เก็บเกี่ยว : <span style="color: red;">&nbsp;*</span></label>
				<input type="date" class="form-control"  name="harvest_date" placeholder="กรอกจำนวนวันที่ปลูก" required /value="<?= $row['harvest_date'];?>">
			</div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
            <img src="img_vegetable/<?php echo $row['img'];?>" width="10%">
            ...
            <center><br><button type="submit" name="vegetable" class="btn btn-outline-success"><i class="bi bi-check-circle"></i> บันทึก</button></center>
